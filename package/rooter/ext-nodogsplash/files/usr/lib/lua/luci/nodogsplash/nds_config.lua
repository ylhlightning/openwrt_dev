...
function NDSConfig:getUci(uciConfig)
   ...
   local iface = uci:get(uciConfig, "interfaces", "interface")
	local iname = uci:get(uciConfig, "interfaces", "name")
	if iname == nil then
		iname = "ROOter Captive Portal"
	end
   table.insert(config, "GatewayInterface "..iface)
   table.insert(config, "GatewayName " .. iname)
   table.insert(config, "\n")
   -- get settings
   ...
   local tUnit = uci:get(uciConfig, "settings", "splashunit")
   local sTime = uci:get(uciConfig, "settings", "splashtime")
   local splashTime = nil
   if tUnit == "days" then
	  splashTime = (tonumber(sTime) * 24 * 60)
   elseif tUnit == "hours" then
	  splashTime = (tonumber(sTime) * 60)
   else
	  splashTime = (tonumber(sTime))
   end
   table.insert(config, "ClientIdleTimeout "..splashTime)
   table.insert(config, "ClientForceTimeout "..splashTime)
   table.insert(config, "\n")
