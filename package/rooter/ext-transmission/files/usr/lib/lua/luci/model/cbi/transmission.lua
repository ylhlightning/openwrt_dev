...
m = Map("transmission", "Transmission Bittorrent Client", translate("Transmission is a simple bittorrent client.") .. button)

m.on_after_commit = function(self)
	enabe = luci.model.uci.cursor():get("transmission", "transmission", "enabled")
	if enabe == "1" then
		os.execute("/etc/init.d/transmission start")
	else
		os.execute("/etc/init.d/transmission stop")
	end
	luci.http.redirect(luci.dispatcher.build_url("admin/services/transmission"))
end
...
mngt=m:section(TypedSection, "transmission", translate("Management of Finished Torrents"))
mngt.anonymous=true

script_torrent_done_enabled=mngt:option(Flag, "script_torrent_done_enabled", translate("Management Enabled"))
script_torrent_done_enabled.enabled="true"
script_torrent_done_enabled.disabled="false"
script_torrent_done_filename=mngt:option(ListValue, "script_torrent_done_filename", translate("Action After Finishing"))
script_torrent_done_filename:depends("script_torrent_done_enabled", "true")
script_torrent_done_filename.default="/etc/torrentdone.sh"
script_torrent_done_filename:value("/etc/torrentdone.sh", "Copy to Folder")
cfolder = mngt:option(Value, "cfolder", translate("Folder"))
cfolder.default="/mnt/sda3"
cfolder.rmempty = true
cfolder:depends("script_torrent_done_enabled", "true")
